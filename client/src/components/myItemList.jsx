...
export default function MyItemList(props) {
  ...
  useEffect(() => {
    ...
    const userid = JSON.parse(jsonPayload).user;
      try{
        ...
        const response = await UserFetch.get(`/${userid}/myitems`,{
          headers: {token: localStorage.token}
        });
        console.log(response.data.data);
        const updatedItems = await Promise.all(response.data.data.myitems.map(async(item) => {
